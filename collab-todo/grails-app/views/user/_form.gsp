...
<div
	class="fieldcontain ${hasErrors(bean: userInstance, field: 'address', 'error')} required">
	<label for="address"> <g:message code="user.address.label"
			default="Address" /> <span class="required-indicator">*</span>
	</label> <a href="#" id="userAddress" metaController="user"
		metaAction="editPropertyAddress">Edit</a>
	<div id="userAddressPlaceholder"></div>
</div>

<div
	class="fieldcontain ${hasErrors(bean: userInstance, field: 'categories', 'error')} ">
	<label for="categories"> <g:message
			code="user.categories.label" default="Categories" />
	...
	<ul class="one-to-many">
		...
		<li class="add"><g:link controller="category" action="create"
				params="['user.id': userInstance?.id]">
				${message(code: 'default.add.label', args: [message(code: 'category.label', default: 'Category')])}
			</g:link></li>
	</ul>
...
<div
	class="fieldcontain ${hasErrors(bean: userInstance, field: 'todos', 'error')} ">
	<label for="todos"> <g:message code="user.todos.label"
			default="Todos" />
	...
	<ul class="one-to-many">
		<g:each in="${userInstance?.todos?}" var="t">
			<li><g:link controller="todo" action="show" id="${t.id}">
					${t?.encodeAsHTML()}
				</g:link></li>
		</g:each>
		<li class="add"><g:link controller="todo" action="create"
				params="['user.id': userInstance?.id]">
				${message(code: 'default.add.label', args: [message(code: 'todo.label', default: 'Todo')])}
			</g:link></li>
	</ul>
